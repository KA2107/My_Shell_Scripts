...
_UEFI_DUET_INSTALLER_GIT() {
	...
	if [[ -e "${_BACKUP_BUILDS_DIR}/SHELLPKG_BUILD/X64/Shell.efi" ]] && [[ "$(file "${_BACKUP_BUILDS_DIR}/SHELLPKG_BUILD/X64/Shell.efi" | grep 'executable')" ]]; then
		rm -f "${_UEFI_DUET_INSTALLER_DIR}/Shell/shellx64_v2.efi" || true
		install -D -m0644 "${_BACKUP_BUILDS_DIR}/SHELLPKG_BUILD/X64/Shell.efi" "${_UEFI_DUET_INSTALLER_DIR}/Shell/shellx64_v2.efi"
	fi
	
	echo
	
	rm -f "${_UEFI_DUET_INSTALLER_DIR}/Shell/shellx64_v1.efi" || true
	install -D -m0644 "${_UDK_DIR}/EdkShellBinPkg/FullShell/X64/Shell_Full.efi" "${_UEFI_DUET_INSTALLER_DIR}/Shell/shellx64_v1.efi"
