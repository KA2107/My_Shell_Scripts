...
_COPY_UEFI_SHELL_UEFI_SYS_PART() {
	...
	if [[ -e "${_BACKUP_BUILDS_DIR}/SHELLPKG_BUILD/X64/Shell.efi" ]] && [[ "$(file "${_BACKUP_BUILDS_DIR}/SHELLPKG_BUILD/X64/Shell.efi" | grep 'executable')" ]]; then
		sudo rm -f "${_UEFI_SYS_PART}/EFI/tools/shellx64_v2.efi" || true
		sudo install -D -m0644 "${_BACKUP_BUILDS_DIR}/SHELLPKG_BUILD/X64/Shell.efi" "${_UEFI_SYS_PART}/EFI/tools/shellx64_v2.efi"
	fi
	
	echo
	
	sudo rm -f "${_UEFI_SYS_PART}/EFI/tools/shellx64_v1.efi" || true
	sudo install -D -m0644 "${_UDK_DIR}/EdkShellBinPkg/FullShell/X64/Shell_Full.efi" "${_UEFI_SYS_PART}/EFI/tools/shellx64_v1.efi"
